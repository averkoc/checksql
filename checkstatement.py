...
def set_translations(my_lang):
    if not my_lang == 'English':
        langfile = str(my_lang) + '.json'
        with open(langfile, encoding='utf-8') as fp:
            global messages
...
class CompareTestResult:
    ...
    def getfeedback(self, index):
        ...
        if self.passlevel in (
                PassLevel.EXECUTED, PassLevel.ROWCOLUMNMATCH, PassLevel.CONTENTMATCH, PassLevel.ORDERMATCH):
...
class StatementChecker:
    ...
    @staticmethod
    def compareSQL(statement: str, refstatement: str, db: str) -> CompareTestResult:
        compare_result = CompareTestResult(statement)
        ...
        try:
            c.execute(statement)
            rows1 = c.fetchall()
            compare_result.passlevel = PassLevel.EXECUTED

            if len(rows1) > 0:
                compare_result.sampleRow = rows1[0]

            c.execute(refstatement)
            rows2 = c.fetchall()
            if len(rows2) > 0:
                compare_result.correctSampleRow = rows2[0]
            ...
            if (len(rows1), columns1) == (len(rows2), columns2):
                compare_result.passlevel = PassLevel.ROWCOLUMNMATCH
                ...
                rows = c.fetchall()
                if len(rows) == 0:
                    compare_result.passlevel = PassLevel.CONTENTMATCH

            if len(rows1) > 0:
                compare_result.samplerow = rows1[0]

        except Error as e:
            compare_result.passlevel = PassLevel.ERROR
            compare_result.errormsg = e.__str__()
            pass
